...
class new {
        ...
        method consume(c) {
            ...
            //TODO: check for "#pragma"
                ...
                match (c)
                    ...
                    case { "," -> emit (commaToken) }
                    case { "\{" -> emit (lBraceToken) }
                    case { "}" ->
                            advanceTo (rBraceState)
                            state.consume(c)
                    }
                    case { "(" -> emit (lParenToken) }
                    case { ")" ->
                    ...
                    case { "[" -> emit (lSquareToken) }
                    case { "]" -> emit (rSquareToken) }
                    case { "⟦" -> emit (lGenericToken) }
                    case { "⟧" -> emit (rGenericToken) }
                    case { ";" -> emit (semicolonToken) }
    ...
    def rBraceState = object {
        method consume (c){
            if (interpdepth > 0) then {
                ...
                advanceTo (quotedStringInterpState)
            }
            else {
                emit (rBraceToken)
                advanceTo (defaultState)
    ...
    def quotedStringInterpState = object {
        method consume (c) {
            if (c == "\n") then {
               advanceTo(newLineState)
               state.consume(c)
